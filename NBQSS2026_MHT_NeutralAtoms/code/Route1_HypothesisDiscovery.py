...
phase contrast, 720x576 px, 300 frames, 1.6 um/px, 10 min between frames.
Cells proliferate from 74 to 661 over the sequence, which is what creates the
association ambiguity we need.  Detections come from the raw images (background
subtraction + connected components); the ground truth is used ONLY to report how
good that detector is, never by the tracker.

Read straight out of the zips -- nothing is extracted to disk.
...
TRAIN_ZIP = "Train Data.zip"
TEST_ZIP = "Test Data.zip"
...
class Cfg:
    ...
    min_area: int = 6
    max_area: int = 400
    match_radius: float = 12.0        # for scoring the detector against GT
...
# ==============================================================================
# Everything is read straight from the zip archives.  The detector is
# deliberately simple: subtract a smooth background, threshold, take connected
# components.  Its imperfections are a feature -- missed detections and false
# positives are exactly the conditions that make MHT necessary.

_ZC: Dict[str, zipfile.ZipFile] = {}


def zopen(path: str) -> zipfile.ZipFile:
    if path not in _ZC:
        _ZC[path] = zipfile.ZipFile(path)
    return _ZC[path]


def read_tif(zip_path: str, member: str) -> np.ndarray:
    with zopen(zip_path).open(member) as f:
        return np.array(Image.open(io.BytesIO(f.read())))


def raw_frame(k: int, cfg: Cfg = CFG, zip_path: str = TRAIN_ZIP) -> np.ndarray:
    return read_tif(zip_path, f"PhC-C2DL-PSC/{cfg.seq}/t{k:03d}.tif")


def gt_centroids(k: int, cfg: Cfg = CFG) -> np.ndarray:
    """Ground-truth cell centres, used ONLY to grade the detector."""
    lab = read_tif(TRAIN_ZIP, f"PhC-C2DL-PSC/{cfg.seq}_GT/TRA/man_track{k:03d}.tif")
    ids = np.unique(lab)
    ids = ids[ids > 0]
    c = ndi.center_of_mass(np.ones_like(lab), lab, ids)
    return np.array([p[::-1] for p in c])          # (x, y)


def detect(img: np.ndarray, cfg: Cfg = CFG) -> np.ndarray:
    """Return an (n, 2) array of detected cell centres in pixels."""
    f = img.astype(float)
    d = f - ndi.gaussian_filter(f, cfg.bg_sigma)   # flatten illumination
    mask = d > cfg.thresh_k * d[d > 0].std()
    mask = ndi.binary_opening(mask, np.ones((2, 2)))
    lab, n = ndi.label(mask)
    if n == 0:
        return np.zeros((0, 2))
    areas = ndi.sum(np.ones_like(lab), lab, range(1, n + 1))
    keep = [i + 1 for i in range(n) if cfg.min_area <= areas[i] <= cfg.max_area]
    if not keep:
        return np.zeros((0, 2))
    c = ndi.center_of_mass(np.ones_like(lab), lab, keep)
    return np.array([p[::-1] for p in c])


def grade_detector(D: np.ndarray, G: np.ndarray, r: float) -> Tuple[float, float]:
    """Greedy nearest-neighbour matching -> (recall, precision)."""
    if len(D) == 0 or len(G) == 0:
        return 0.0, 0.0
    used, tp = set(), 0
    for g in G:
        dist = np.linalg.norm(D - g, axis=1)
        j = int(np.argmin(dist))
        if dist[j] <= r and j not in used:
            used.add(j)
            tp += 1
    return tp / len(G), tp / len(D)


FRAMES = [CFG.frame0 + i * CFG.stride for i in range(CFG.window)]
assert FRAMES[-1] <= 299, "window runs past the end of the sequence"

DETECTIONS = [detect(raw_frame(k)) for k in FRAMES]

print(f"Sequence {CFG.seq}, frames {FRAMES}  (stride {CFG.stride} = "
      f"{CFG.stride * 10} min between used frames)")
print("\n frame | detected |   GT | recall | precision")
for k, D in zip(FRAMES, DETECTIONS):
    G = gt_centroids(k)
    rec, prec = grade_detector(D, G, CFG.match_radius)
    print(f"  {k:4d} | {len(D):8d} | {len(G):4d} | {rec:6.2f} | {prec:9.2f}")
...
detector (recall ~0.5, precision ~0.85), never by the tracker.
